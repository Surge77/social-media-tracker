    ...
    <section className="relative overflow-hidden py-8 sm:py-10">
      ...
      <ScrollVelocityContainer className="flex flex-col gap-4">
        ...
        <ScrollVelocityRow
          baseVelocity={3}
          direction={1}
          className="text-lg font-semibold tracking-tight text-muted-foreground/50 sm:text-2xl"
        >
          {row1.map((item) => (
        ...
        <ScrollVelocityRow
          baseVelocity={3}
          direction={-1}
          className="text-lg font-semibold tracking-tight text-foreground/20 sm:text-2xl"
        >
          {row2.map((item) => (
            <span key={item} className="inline-flex items-center">
              <span className="whitespace-nowrap">{item}</span>
